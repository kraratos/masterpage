...
import { CpUserManagementApiService } from 'src/services/api/cp-ums-api.service';
import { IHierarchyLevel } from '@bakerhughes-icenter/assessment-search-app-lib/lib/interface/response/hierarchy-response';
...
export class MicroAppLayoutComponent implements OnInit {
  ...
  constructor(
    ...
    private userService: CpUserManagementApiService
  ) { }

  ngOnInit(): void {

    this.userService.fetchMeHierarchy().subscribe({
      next: (data) => {
        console.log("User hierarchy", data)
        if (data) {
          // this.dataService.setHierarchy(data as Array<IHierarchyLevel>);
        }
      },
    ...
    this.userService.fetchMeDetails().subscribe({
      next: (data) => {
        console.log("User details", data)
        if (data && !__.isEmpty(data)) {
    ...
    this.userService.fetchMePrivileges().subscribe({
      next: (data) => {
        console.log("User privileges", data)
        if (data && !__.isEmpty(data)) {
          // this.dataService.setUserPrivileges(data);
        }
      },
      error: (e) => console.error(e),
      complete: () => { }
    ...
    console.log("static parameter 1 : " + assessmentId)
    // oppure in modo reattivo:
    this.route.paramMap.subscribe(params => {
      const assessmentId = params.get('assessmentId');
      console.log("static parameter 2 : " + assessmentId)
    });

    this.route.queryParamMap.subscribe(params => {
      const assessmentId = params.get('assessmentId');
      console.log("dynamic parameter 1 : " + assessmentId)
    });
    ...
    if (getContext?.context) {
      ...
      if (readContext) {
        ...
        if (this.microAppService.selectedVid != data.vid) {
          ...
          this.microAppService.selectedId = (payload.vid as string).substring(indexOfUnderScore + 1, (payload.vid as string).length);
