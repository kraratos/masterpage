...
import { async, BehaviorSubject, catchError, forkJoin, from, map, mergeMap, Observable, of, ReplaySubject, Subject, Subscription, switchMap, takeUntil, tap } from "rxjs";
...
export class WidgetsService extends HttpAbstractBackendService {
    ...
    cpDashboardDataService: CpDashboardApiService;
    widgetDataReadyOservable: BehaviorSubject<Map<number, WidgetData>> = new BehaviorSubject(new Map());
    sigleWidgetReaday: BehaviorSubject<any> = new BehaviorSubject<any>(undefined);
    layoutDataSingleApp: BehaviorSubject<any> = new BehaviorSubject<any>(undefined);
    ...
    constructor(http: HttpClient, public dataService: DataService) {
        ...
        this.cpDashboardDataService = new CpDashboardApiService(http);
    }
    ...
    updateLayoutWidgetData(
        ...
        this.widegtForUserSubcription = this.cpDashboardDataService.getWidgetsForUser({
            vid,
            pageName: page,
    ...
    // updateWidgeData(page: Pages, vid: string, dataRange: string, assetLevel: string, fromDate: Date, currentDate: Date, layoutData: any, contextLevel: string, microappName: string = MASTERPAGEAPP, response: any) {
    ...
    //     //this.prapareWidgetListForPage(this.widgetsMap, fromDate, currentDate, layoutData, contextLevel, dataRange);
